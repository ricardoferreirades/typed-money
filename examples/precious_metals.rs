...
use typed_money::{Amount, Currency, XAU, XAG, XPT, XPD, XDI, XCU, XAL, XZN, XNI};
...
fn main() {
    ...
    println!("Silver: {} ({} troy ounces)", silver, silver.to_major_floor());
    
    // Platinum (XPT) - troy ounces
    let platinum = Amount::<XPT>::from_major(2);
    println!("Platinum: {} ({} troy ounces)", platinum, platinum.to_major_floor());
    
    // Palladium (XPD) - troy ounces
    let palladium = Amount::<XPD>::from_major(5);
    println!("Palladium: {} ({} troy ounces)", palladium, palladium.to_major_floor());
    ...
    println!("Copper: {} ({} metric tons)", copper, copper.to_major_floor());
    
    // Aluminum (XAL) - metric tons
    let aluminum = Amount::<XAL>::from_major(50);
    println!("Aluminum: {} ({} metric tons)", aluminum, aluminum.to_major_floor());
    ...
    println!("Nickel: {} ({} metric tons)", nickel, nickel.to_major_floor());
    
    println!("\n=== Currency Properties ===");
    println!("Gold (XAU): {} decimals, code: {}, symbol: {}", XAU::DECIMALS, XAU::CODE, XAU::SYMBOL);
    println!("Silver (XAG): {} decimals, code: {}, symbol: {}", XAG::DECIMALS, XAG::CODE, XAG::SYMBOL);
    println!("Diamond (XDI): {} decimals, code: {}, symbol: {}", XDI::DECIMALS, XDI::CODE, XDI::SYMBOL);
    println!("Copper (XCU): {} decimals, code: {}, symbol: {}", XCU::DECIMALS, XCU::CODE, XCU::SYMBOL);
    ...
    println!("Silver multiplication: {} * 3 = {}", silver_amount, multiplied_silver);
    
    println!("\n=== Precision Testing ===");
    
    // Test 4 decimal precision
    let precise_gold = Amount::<XAU>::from_minor(12345); // 1.2345 troy ounces
    println!("Precise gold: {} (minor units: {})", precise_gold, precise_gold.to_minor());
    
    let precise_copper = Amount::<XCU>::from_minor(56789); // 5.6789 metric tons
    println!("Precise copper: {} (minor units: {})", precise_copper, precise_copper.to_minor());
}
