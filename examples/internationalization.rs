...
use typed_money::{
    Amount, Currency, CurrencyMetadata, CurrencyType, SymbolPosition, 
    VolatilityRating, LiquidityRating, USD, EUR, GBP, JPY, CNY, BRL, INR, ZAR, XAU, BTC
};
...
fn format_locale_specific<C: Currency>(amount: Amount<C>) -> String {
    ...
    let amount_str = if C::DECIMALS > 0 {
        format!("{}{}{}", formatted_major, C::DECIMAL_SEPARATOR, formatted_minor)
    } else {
        formatted_major
    };
...
fn analyze_currency_type(name: &str, currency_type: CurrencyType, is_major: bool, is_stable: bool) {
    println!("{}: {} - Major: {}, Stable: {}", name, currency_type, is_major, is_stable);
}
...
fn group_currencies_by_region() {
    ...
    let mut regions: std::collections::HashMap<String, Vec<&str>> = std::collections::HashMap::new();
    
    for (currency, region) in currencies {
        regions.entry(region.to_string()).or_insert_with(Vec::new).push(currency);
    }
...
fn analyze_currency_for_trading<C: Currency>(name: &str, amount: Amount<C>) {
    ...
    let recommendation = match (amount.volatility_rating(), amount.liquidity_rating(), amount.is_stable_currency()) {
        (VolatilityRating::Low, LiquidityRating::High, true) => "Excellent for conservative trading",
